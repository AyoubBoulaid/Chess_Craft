...
class Game:
    def __init__(self, engine_path, player_color=chess.WHITE, vs_ai=True):
        self.engine_path = engine_path
        self.vs_ai = vs_ai
        self.engine = chess.engine.SimpleEngine.popen_uci(self.engine_path) if vs_ai else None
    ...
    def draw_game_over(self, screen):
        ...
        # Draw the Hint button on the overlay
        draw_button(overlay, "Hint", SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT // 2 + 110, 100, 50,
                (255, 255, 255), (200, 200, 200), self.show_hint)

        # Blit (draw) the overlay onto the screen
        screen.blit(overlay, (0, 0))

    def show_hint(self):
        """Shows a hint for the next best move."""
        if not self.game_over and self.board.turn == self.player_color:
            result = self.engine.play(self.board, chess.engine.Limit(time=1))
            hint_move = result.move
            print(f"Hint: {hint_move.uci()}")
...
def draw_button(screen, text, x, y, width, height, color, hover_color, action=None):
    ...
    draw_button(overlay, "Undo", SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT // 2 + 170, 100, 50,
                (255, 255, 255), (200, 200, 200), self.undo_move)
...
def main():
    ...
    pygame.display.set_caption("Chess_Craft By A-U-B Using Stockfish ai")

    engine_path = "C:\\Users\\Asus\\Desktop\\Chess_Craft\\stockfish\\stockfish.exe"
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if game.game_over:
                    if draw_button(screen, "Restart", SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT // 2 + 50, 100, 50,
                               (255, 255, 255), (200, 200, 200), game.reset):
                        game.reset()
                    else:
                        game.handle_mouse_click(pos)
                    if draw_button(screen, "Undo", SCREEN_WIDTH - 110, SCREEN_HEIGHT - 50, 100, 50, UNDO_COLOR, (200, 0, 0), game.undo_move):
                        game.undo_move()
                    if draw_button(screen, "Hint", SCREEN_WIDTH - 110, SCREEN_HEIGHT - 110, 100, 50, HINT_COLOR, (0, 200, 0), lambda: show_hint(screen, game)):
                        pass
